...
class Empty extends TweetSet {
  ...
  def mostRetweeted: Tweet = throw new NoSuchElementException("There is no tweet!")
...
class NonEmpty(elem: Tweet, left: TweetSet, right: TweetSet) extends TweetSet {
  ...
  def mostRetweeted: Tweet = {
    var res = elem
    if (!left.isEmpty) res = {
      val temp = left.mostRetweeted
      if (temp.retweets>res.retweets) temp
      else res
    }
    if (!right.isEmpty) res = {
      val temp = right.mostRetweeted
      if (temp.retweets>res.retweets) temp
      else res
    }
    res
